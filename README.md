...
- Автоперезагрузка просмотра при изменении файла на диске (в том числе после
  правки другим редактором); работает и на дисках WSL/сетевых, где системный
  «watcher» молчит, — за счёт фонового опроса времени изменения.
- Кнопка **⟳ Обновить** над деревом (и `Ctrl+R`) — перечитать открытый файл и
  дерево с диска вручную.
- Контекстное меню на вкладке (правая кнопка): скопировать путь к папке файла,
  полный путь с именем файла или только имя файла.
...
- Дерево слева показывает все корни верхним уровнем — локальные диски, флешки,
  **сетевые диски** и дистрибутивы **WSL** (`\\wsl.localhost\Ubuntu`), — а не только буквы дисков.
  Каталоги с точкой в имени (например `.tmp`) отображаются и доступны для навигации.
